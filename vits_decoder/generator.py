...
class Generator(torch.nn.Module):
    ...
    def __init__(self, hp):
        ...
        self.pre_style = nn.Linear(hp.vits.gin_channels, hp.gen.upsample_input)
    ...
    def forward(self, spk, x, f0, s):
        # Perturbation
        x = x + torch.randn_like(x)
        
        # Style information
#         print(s.shape, x.shape)
        x = x.permute(0,2,1) + self.pre_style(s.squeeze(-1)).unsqueeze(1)
        x = x.permute(0,2,1)
        # adapter
    ...
    def inference(self, spk, x, har_source, s):
        # adapter
